...
def rescale_intensity(img):
    img = skimage.img_as_float(img)
    img = exposure.rescale_intensity(img, in_range=(0.1, 0.9))
    img = skimage.img_as_ubyte(img)
    return img
...
def cut_img_center(img, factor=0.05):
    width, height = img.size
    crop_rectangle = (factor * width,
                      factor * height,
                      width - factor * width,
...
def get_video_rows(path, feature_extractor, preprocessor,
    ...
    for frame in container.decode(stream):
        frame_idx += 1
        if mod_filter and frame_idx % mod_filter != 0:
            continue
        img = frame.to_image()
        if ssim_threshold is not None and prev_img:
            if compare_ssim(np.array(img), np.array(prev_img), multichannel=True) > ssim_threshold:
                continue
        prev_img = img
        img = preprocessor(img)
        features = feature_extractor(rgb_to_bgr(np.array(img)))
        row = [path, round(frame.time, 1)]  # metadata, features
        for i in range(len(features)):
            row.append(features[i])
        rows.append(tuple(row))
        if frames_path:
            fpath = os.path.join(frames_path, '{}.jpg'.format(str(round(frame.time,2))))
            Image.fromarray(img).save(fpath)
    return rows


def get_dataframe(fpaths, feature_extractor, preprocessor=preprocess_image_load,
                  write_frames_dir=None, mod_filter=None, ssim_threshold=None):
    rows = []
    i = 0
    for fpath in fpaths:
